...
test('parseDiagnosticJsonl links decisions to raw points and builds summary', () => {
  const model = parseDiagnosticJsonl([
    ...
    '{"event":"decision","decisionId":2,"rawPointId":2,"result":"reject","reason":"impossible_speed"}'
  ].join('\n'));
  ...
  assert.equal(model.points[1].decision.reasonExplanation.title, '不可能速度跳点');
  assert.equal(model.reasonCounts[1].explanation.title, '不可能速度跳点');
  assert.deepEqual(model.reasonCounts.map((item) => item.reason), [
    'anchor:first_fix_good',
    'reject:impossible_speed'
  ]);
});
...
test('parseDiagnosticJsonl builds diagnostic evidence findings and point insights', () => {
  const model = parseDiagnosticJsonl([
    ...
    '{"event":"decision","decisionId":1,"rawPointId":1,"result":"anchor","reason":"first_fix_good","segmentId":1,"distanceDeltaMeters":0,"movingTimeDeltaSeconds":0,"sourceGnssSnapshotId":7}',
    '{"event":"raw_location","rawPointId":2,"provider":"gps","lat":29.001,"lng":106,"accuracy":35,"elapsedRealtimeNanos":12000000000,"sourceGnssSnapshotId":7,"gnssQualityStale":true,"sourceGnssSnapshotAgeNanos":10500000000}',
    '{"event":"decision","decisionId":2,"rawPointId":2,"result":"weak","reason":"weak_signal_stage1","distanceDeltaMeters":0,"movingTimeDeltaSeconds":0,"sourceGnssSnapshotId":7}',
...
test('explainDecisionReason returns Chinese reason guidance with fallback', () => {
  const weak = explainDecisionReason('weak', 'weak_signal_stage1');
  assert.equal(weak.title, '移动中弱信号');
  assert.ok(weak.evidence.includes('usedAvgCn0'));
