...
Web 还会生成 `adaptiveShadow`。它使用 `sessionProfile` 派生一组旁路阈值并重算一次，
记录整条轨迹统计差值、影子轨迹预览，以及与当前固定阈值结果不同的 raw 点。
`adaptiveShadow` 不替换 `target track`，不改变里程、GAP、segment、decision reason 或
GPX 输出，只用于发现“如果自适应会改变整条轨迹多少、会改变哪些点、地图上会改在哪里”。
页面应能从分歧列表直接定位到地图 raw 点，避免多文件、多分歧场景下只靠肉眼扫图。

`adaptiveShadow.assessment` 只做启用前诊断：如果影子会降级当前可信点、明显改变里程、
改变 GAP / 疑似交通数量或分歧比例过高，则标记为需要复核或暂不适合启用。这个判断不自动启用
自适应阈值，也不改变当前固定阈值成品。

当一次导入多个 evidence 文件时，Web 页面可以按文件聚合 `adaptiveShadow.assessment`，
展示 `same / observe / review / blocked` 分布、累计影响和优先复核文件，用于判断自适应阈值
是否具备进入候选策略的样本基础。
...
`lowQualityMotionRebuild.rawIntervalCandidates`。这个扫描不要求后一个目标点必须是
`stationary_anchor / gap_recovery / continuity_rescue_gap_recovery`，而是按 raw
时间序列查找满足同一组持续时间、active-motion、步距、采样间隔和 bbox 条件的连续区间，
并记录该区间中已有多少 raw 点被解释为 trusted / weak / reject / unexplained。

广义 raw 区间只用于发现“旧可入轨入口没有触发，但原始时间序列里确实有连续运动线索”的样本。
它不会被 `lowQualityMotionRebuildEnabled` 直接写入成品轨迹；进入轨迹仍必须先经过更明确的
轨迹结构规则和人工复核。

### 10. 气压阻止静止整段压缩

气压证据默认不参与清洗，Web 页面提供 `气压阻止静止整段压缩` 开关。开启后，气压仍然
不作为剔除 raw 点的硬规则，只作为静止整段压缩的反证。
