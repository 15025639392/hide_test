# 系统 GNSS 定位与轨迹记录测试方案（归档长版）

> 本文件是瘦身前的详细设计归档，保留历史模型、长期设想和测试矩阵。
> 当前日常开发与验收以 `docs/system-gnss-track-recording-plan.md` 为准。
...
当前实现优先服务真实徒步记录，而不是只做实验室策略验证：

```text
最终用户看到的是一条连续轨迹线:
  可信 TrackPoint 按时间连续连接
  GAP 后恢复点仍连接到最终轨迹线中
  不用视觉断口把用户的完整徒步拆成多条碎线

系统内部必须保留 GAP 语义:
  超过 120 秒的可信点间隔 -> decisionReason = gap_recovery
  恢复点进入 TrackPoint，但 distanceDeltaMeters = 0
  movingTimeDeltaSeconds = 0
  segmentId 增加，gapCount 增加
  该连接只用于最终轨迹连续展示，不算可信距离

可信距离与最终轨迹线分离:
  totalDistanceMeters 只累计可信移动点之间的距离
  GAP 两端直线不计入可信距离
  GPX extension 保留 decisionReason / segmentId / distanceDeltaMeters 供复盘
```

这意味着 `segmentId` 是诊断和统计语义，不等同于最终展示必须断线。真实徒步产品更需要一条完整轨迹线；可解释性通过诊断字段、`gapCount`、`gap_recovery` 和距离不累计来保证。

### GPX 导出基于正式轨迹点

GPX 不应导出全部原始点，而应导出过滤后的正式轨迹点。

当前 M6 口径下，可信 GPX 默认保持连续 `<trkseg>`，让常见地图工具呈现一条完整徒步线。长时间无定位、暂停恢复、严重跳点后恢复时，不再依赖 `<trkseg>` 断开表达异常，而是通过每个 `<trkpt>` 的 extension 写入 `segmentId`、`decisionReason = gap_recovery`、`distanceDeltaMeters = 0`、`movingTimeDeltaSeconds = 0` 来表达。

`partial.gpx` 仍可把 weak 点单独表达为诊断段，用于排查弱信号环境；它不是最终可信轨迹的展示口径。
...
location.elapsedRealtimeNanos <= nowElapsedRealtimeNanos + toleranceNanos
...
如果定位点的 `elapsedRealtimeNanos` 明显来自未来：

```text
location.elapsedRealtimeNanos > nowElapsedRealtimeNanos + toleranceNanos
rejectReason = location_from_future
```
...
真实徒步模式(M5):
  ...
  每次策略切换必须写入 sampling_policy 诊断事件
```
...
这个策略用时间间隔做功耗折中，用 App 层决策做距离和可信度判断，不把距离过滤交给 Android 系统。
...
前台服务不是“定位更准”的来源，它只提高长时间记录的存活性和回调连续性。定位质量仍由系统 GNSS、设备天线、遮挡、星历状态和环境决定。服务运行中如果出现长 gap，最终轨迹线可以连续展示，但必须按 `GAP_RECOVERY` 记录诊断语义，并且不得把 gap 两端直线计入可信距离。

真实徒步前台服务当前使用 `START_REDELIVER_INTENT`。如果进程被系统杀掉，系统有机会用最近一次 start intent 重新投递；仍需通过 `session.json`、`diagnostic.jsonl`、`completionState` 和 `recoveryState` 判断这次记录是否完整。
...
long_gap_recovery:
  ...
  期望新内部 TrackSegment，恢复点 reason = gap_recovery，distanceDelta = 0
...
12. 如果 result = accept:
   ...
   - 如果 reason = gap_recovery，distanceDelta/movingTimeDelta 保持 0
   - 如果 reason = gap_recovery，允许 startsNewSegment = true
13. 如果 result = anchor:
   - 更新 TrackSegment
   - distanceDelta = 0
   - movingTimeDelta = 0
...
accept:
  ...
  gap_recovery 是 accept 的特例，进入 TrackPoint 但 distanceDelta/movingTimeDelta 为 0
...
新 TrackSegment 的第一个 GPX 点如果是 GAP 恢复点，当前实现使用 `decisionResult = accept` 和 `decisionReason = gap_recovery`，但 `distanceDeltaMeters` / `movingTimeDeltaSeconds` 必须为 0。它是“进入最终连续轨迹线、但不贡献可信距离”的特殊 accept。
...
当前 M6 可信 GPX 默认导出为连续 `<trkseg>`，即使内部 `segmentId` 因 GAP 恢复而增加。这样常见地图工具会显示一条连续徒步线。

内部 `TrackSegment` 不直接等价为 GPX `<trkseg>`。`segmentId`、`decisionReason` 和 delta 字段用于解释可信距离、GAP 和恢复点：
...
<trkseg>
  <trkpt lat="..." lon="...">
    <extensions>
      ...
      <hike:decisionReason>gap_recovery</hike:decisionReason>
      <hike:distanceDeltaMeters>0.0</hike:distanceDeltaMeters>
      <hike:movingTimeDeltaSeconds>0.0</hike:movingTimeDeltaSeconds>
    </extensions>
  </trkpt>
</trkseg>
```

只有 partial GPX 中的 weak 诊断点可以为了表达低可信观测而单独成段。可信 GPX 不用断段来表达 GAP。
...
{"event":"decision","sessionId":"S1","eventSeq":24,"schemaVersion":1,"eventElapsedRealtimeNanos":...,"writtenWallTimeMillis":...,"decisionId":1003,"rawPointId":43,"trackPointId":18,"segmentId":2,"result":"accept","reason":"gap_recovery","startsNewSegment":true,"distanceDeltaMeters":0,"movingTimeDeltaSeconds":0}
...
- 是否出现明显跳点:
- 最终轨迹线是否连续:
- GAP 是否被标记为 gap_recovery / gapCount:
- 距离是否异常增长:
- 和肉眼路线是否一致:
...
恢复点 reason = gap_recovery，distance/movingTime delta 为 0
GPX / 地图最终轨迹线保持连续
...
TrackDecisionEngineTest:
  ...
  长 gap 后恢复生成 gap_recovery，且 delta 为 0
...
GpxExporterTest:
  ...
  多 TrackSegment -> GPX 保持连续 trkseg，并通过 extension 保留 segmentId / gap_recovery
...
ReplayRunnerTest:
  ...
  long_gap_recovery fixture -> 新内部 segment + gap_recovery + delta 0
...
pause_resume_walk:
  记录中暂停 1~2 分钟再恢复移动
  验证恢复语义、距离不跨暂停累计、最终轨迹线连续

gap_recovery_walk:
  进入遮挡区域造成长 gap 后回到开阔地
  验证新内部 segment、gapCount、gap_recovery 和连续 GPX
```
...
- 最终可信 GPX / 地图轨迹线连续展示
- 不同内部 TrackSegment 之间允许连成最终轨迹线，但必须通过 `gap_recovery` / `gapCount` / `distanceDeltaMeters=0` 保留诊断语义
- GPX 可被常见地图工具打开
- 静止 2 分钟距离不应明显增长
- 记录开始前缓存点不会进入 GPX
